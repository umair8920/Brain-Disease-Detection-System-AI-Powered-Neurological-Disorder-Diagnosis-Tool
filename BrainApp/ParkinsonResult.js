...
const ParkinsonResult = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('http://10.110.41.71:5000/api/get_resized_parkinson_image', {
        method: 'GET',
        headers: {
  ...
  <View style={styles.overlay}>
    ...
    <View style={styles.container2}>
      <Text style={styles.heading}>Below is the prediction result for Parkinson's disease:</Text>
      </View>
      {isLoading ? (
        ...
        <>
          ...
          />
        <View style={styles.container2}>
          <Text>Classification: {resultData.parkinson_classification_results.classification}</Text>
          <Text>Confidence: {resultData.parkinson_classification_results.confidence}%</Text>
        </View>
        </>
      ) : (
...
export default ParkinsonResult;
